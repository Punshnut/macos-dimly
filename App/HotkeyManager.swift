...
struct HotkeyDescriptor: Equatable, Hashable, Codable {
    ...
    static let panicDefault = HotkeyDescriptor(
    ...
    var displayString: String {
        ...
        let keyText = keyDisplayText().isEmpty ? Self.unknownKeyLabel(for: keyCode) : keyDisplayText()
    ...
    private func modifierDisplayParts() -> [String] {
        var parts: [String] = []
        if modifierFlags.contains(.command) {
            parts.append(Self.localizedLabel("Cmd"))
        }
        if modifierFlags.contains(.option) {
            parts.append(Self.localizedLabel("Option"))
        }
        if modifierFlags.contains(.shift) {
            parts.append(Self.localizedLabel("Shift"))
        }
        if modifierFlags.contains(.control) {
            parts.append(Self.localizedLabel("Ctrl"))
        }
        return parts
    }
    ...
    private func keyDisplayText() -> String {
        ...
        if let keyRepresentation {
            return Self.localizedLabel(keyRepresentation)
        }
...
final class HotkeyManager {
    ...
    func activate() -> Bool {
        ...
        guard let registeredHotkey else { return false }
        ...
        if didRegisterHotkey {
            ...
            DiagnosticsLogger.shared.log("Failed to register global hotkey", category: "hotkey")
        }
